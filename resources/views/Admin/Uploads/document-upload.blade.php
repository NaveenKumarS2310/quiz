    ...
    <div class="content-wrapper">
        <div class="page-header">
            <h3 class="page-title">
                ...
                </span> Document Upload - @if (request()->route()->getName() == 'admin.document.upload.edit.index')
                    Update
                @else
                    Create
                @endif
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        @if (request()->route()->getName() == 'admin.document.upload.edit.index')
                            <form action="{{ route('admin.document.upload.edit.store') }}" method="POST"
                                enctype="multipart/form-data">
                            @else
                                <form class="forms-sample" action="{{ route('admin.document.upload.create.store') }}"
                        ...
                        <div class="form-group">
                            ...
                            <input type="file" id="docUploader" accept=".pdf,.doc,.docx" hidden>
                        </div>
                ...
                <div class="col-md-4 mb-4">
                    <div class="card h-100 shadow-sm">
                        ...
                        <div class="card-footer d-flex justify-content-between">
                            ...
                            <a href="{{ route('admin.document.upload.edit.index', ['id' => $article->id, 'page' => isset($_GET['page']) ? $_GET['page'] : 1]) }}"
...
@section('js')
    ...
    <script>
        (function() {
            ...
            function DocButton(context) {
                let ui = $.summernote.ui;

                return ui.button({
                    contents: '<i class="note-icon-file"></i> File',
                    tooltip: 'Upload PDF / DOC',
                    click: function() {
                        $('#docUploader').click();
                    }
                }).render();
            ...
            $('#docUploader').on('change', function() {
...
`; */
    ...
    <script>
        ...
        function categoryChangeConfirmed(id, status, el) {
            $.ajax({
                url: "{{ route('admin.document.upload.status.update') }}",
                type: "POST",
                data: {
                    id: id,
                    status: status,
        ...
        $('#saveArticle').click(function() {
            $.ajax({
                @if (request()->route()->getName() == 'admin.document.upload.edit.index')
                    url: "{{ route('admin.document.category.edit.store') }}",
                @else
                    url: "{{ route('admin.document.category.create.store') }}",
                @endif
                type: "POST",
                data: {
                    id: id,
