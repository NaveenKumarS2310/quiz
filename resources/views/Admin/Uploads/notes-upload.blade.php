    ...
    <div class="content-wrapper">
        ...
        <div class="row">
            <div class="col-md-12 grid-margin stretch-card">
                <div class="card">
                    <div class="card-body">
                        <h4 class="card-title">Notes Upload</h4>
                        ...
                        @endif
                        @if (request()->route()->getName() == 'admin.notes.upload.edit.index')
                            <form action="{{ route('admin.notes.upload.edit.store') }}" method="POST"
                                enctype="multipart/form-data">
                            @else
                                <form class="forms-sample" action="{{ route('admin.notes.upload.create.store') }}"
...
@section('js')
    ...
    <script>
        ...
        function categoryChangeConfirmed(id, status, el) {
            $.ajax({
                url: "{{ route('admin.notes.upload.status.update') }}",
                type: "POST",
                data: {
                    id: id,
                    status: status,
        ...
        $('#saveArticle').click(function() {
            $.ajax({
                @if (request()->route()->getName() == 'admin.notes.upload.edit.index')
                    url: "{{ route('admin.notes.category.edit.store') }}",
                @else
                    url: "{{ route('admin.notes.category.create.store') }}",
                @endif
                type: "POST",
                data: {
                    id: id,
