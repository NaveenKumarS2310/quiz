    ...
    <div class="content-wrapper">
        <div class="page-header">
            <h3 class="page-title">
                ...
                </span> Job Upload - @if(request()->route()->getName() == 'admin.job.upload.edit.index') Update @else Create @endif
            </h3>
        ...
        <div class="row">
            <div class="col-md-12 grid-margin stretch-card">
                <div class="card">
                    <div class="card-body">
                        <h4 class="card-title">Job Upload</h4>
                        ...
                        @endif
                        @if (request()->route()->getName() == 'admin.job.upload.edit.index')
                            <form action="{{ route('admin.job.upload.edit.store') }}" method="POST"
                                enctype="multipart/form-data">
                            @else
                                <form class="forms-sample" action="{{ route('admin.job.upload.create.store') }}"
        ...
        {{-- ================= JOB LIST ================= --}}
        <div class="row mt-4 job-list">
            <div class="col-12 mb-3">
                <h4 class="card-title">Job List</h4>
                <h4 class="card-title float-end">Total Jobs : {{ $articles->total() }}</h4>
                ...
                <div class="col-md-4 mb-4">
                    <div class="card h-100 shadow-sm">
                        ...
                        <div class="card-footer d-flex justify-content-between">
                            ...
                            <a href="{{ route('admin.job.upload.edit.index', ['id' => $article->id,'page'=> isset($_GET['page']) ? $_GET['page'] : 1 ]) }}"
...
@section('js')
    ...
    <script>
        ...
        function categoryChangeConfirmed(id, status, el) {
            $.ajax({
                url: "{{ route('admin.job.upload.status.update') }}",
                type: "POST",
                data: {
                    id: id,
                    status: status,
        ...
        $('#saveArticle').click(function() {
            $.ajax({
                @if (request()->route()->getName() == 'admin.job.upload.edit.index')
                    url: "{{ route('admin.job.category.edit.store') }}",
                @else
                    url: "{{ route('admin.job.category.create.store') }}",
                @endif
                type: "POST",
                data: {
                    id: id,
