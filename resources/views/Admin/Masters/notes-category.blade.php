    ...
    <div class="content-wrapper">
        ...
        <div class="row">
            <div class="col-md-12 grid-margin stretch-card">
                <div class="card">
                    <div class="card-body">
                        <h4 class="card-title">Notes Category</h4>
                        <p class="card-description"> create Or update </p>
                        
                        @if(request()->route()->getName() == "admin.notes.category.edit.index")
                        <form class="forms-sample" action="{{ route('admin.notes.category.edit.store') }}" method="POST">
                         @else
                         <form class="forms-sample" action="{{ route('admin.notes.category.create.store') }}" method="POST">
                         @endif
                            @csrf
                            ...
                            <tbody>
                                @foreach ($categories as $data)
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <a href="{{ route('admin.notes.category.edit.index',['id' => $data->id]) }}"><button type="button" class="btn btn-gradient-info btn-fw">Edit</button></a>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
...
@section('js')
    ...
    ev.preventDefault();       // 2️⃣ stop default toggle
        ...
        function categoryChangeConfirmed(id,status,el){
         ...
         $.ajax({
                url: "{{ route('admin.notes.category.status.update') }}",
                type: "POST",
                data: {
                    id: id,
                    status: status,
